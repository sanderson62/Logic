...
static char *originalLocale = NULL; /* locale before entering routine */
...
void cvlwrupr(unsigned char *pinchar, int plength)
{
   ...
   char *environmentLocale = NULL; /* environment locale string value */
   int environmentLocaleLen = 0; /* environment locale's length */
   int noBytes;
   int retInt;
   int lerr = 0;
   ...
   if (localesChecked == 0) { /* 1st time through? */
      ...
      if (retChar) {
         ...
         originalLocale = malloc(originalLocaleLen + 1);
         if (originalLocale != NULL) { /* malloc gave us memory we asked for */
            strcpy(originalLocale, retChar);
            origlocale_saved = 1;
            retChar = setlocale(LC_CTYPE, "");  /* set locale to local environment */
            if  (retChar) {
               environmentLocaleLen = strlen(retChar);
               environmentLocale = malloc(environmentLocaleLen + 1);
               if (environmentLocale != NULL) { /* malloc gave us memory we asked for */
                  strcpy(environmentLocale, retChar);
               }
               if (environmentLocaleLen != originalLocaleLen)
                  /*
                   * note that both can be == 0
                   */
               {
                  localesDiffer = 1; /* original / environment locales differ */
               } else if ((originalLocaleLen > 0) && /* environmentLocaleLen can't be 0 */
                          (memcmp(environmentLocale, originalLocale, originalLocaleLen) != 0)) {
                  localesDiffer = 1; /* original / environment locales differ */
               }
               /* else */
               /*    Both lengths are the same and their locale values match */
               /*    Keep originalLocale for kxprtd messages */
               if (environmentLocale != NULL) {
                  free(environmentLocale);   /* done w/ it */
               }
            } else {
               kxerror(E_3655, E_PRINT, "cvlwrupr");
            }
         } else {
            kxerror2(E_3662, E_PRINT, "cvlwrupr", originalLocaleLen + 1, errno);
         }
      } else {
         ...
         if (noBytes == 1) { /* single byte */
            ...
            if (!lerr) {
               ...
               */
               ...
               if (origlocale_saved && (tctlocaleDiffers || localesDiffer)) {
