...
// A utility that sets field bld_file to "YES"
function setbld()
{
	CreateSS.bld_file.value = "YES";
}
...
function verify(f)
{
    ...
    //  comp_id selectedindex of 0 is AHL, 1 is CID, 2 is DCC
    for(var i = 0; i < f.length; i++) {
        var e = f.elements[i];
        //  comp_id selectedindex of 0 is AHL, 1 is CID, 2 is DCC
        //  we calcuate the mort reserves for AHL in pgm BSSRCH
        //  and DCC does not require mortality reserves
        if ((e.name == "comp_id") && (e.selectedIndex == 0 || e.selectedIndex == 2)) return true;
