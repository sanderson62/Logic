...
typedef enum {
   ...
   DR_COMMAND_START,
   DR_COMMAND_CANCEL,
   DR_COMMAND_OPEN_OUTPUT,
   DR_COMMAND_CLOSE_OUTPUT,
   DR_GET_RMT_ALT_INFO
} kxdynrte_command_t;
...
#define drGetVsamKeyoffset        (dr->drUnion.vsam.keyoffset)
   ...
   union {
      ...
      */
      struct {
         ...
         char *ridfld;
         int  keylength;
         int  keyoffset;	/* only used for fabricated OPEN OUTPUT FS */
      } vsam;
