      ...
      ***************
       ...
       77  C-FN-SIGNON_CESN                PIC X(2)  VALUE X"7402".
       77  C-FN-SIGNON_CESL                PIC X(2)  VALUE X"0e0e".
       ...
       77  C-CESN-MAP-NAME                 PIC X(8)  VALUE 'CESNMAP'.
       77  C-CESL-MAP-NAME                 PIC X(8)  VALUE 'CESLMAP'.
       77  C-CSSN-MAP-NAME                 PIC X(8)  VALUE 'CSSNMAP'.
       77  C-MAP-SET-NAME                  PIC X(8)  VALUE 'CSIGNMP'.
       77  C-CESN-TRAN-ID                  PIC X(4)  VALUE 'CESN'.
       77  C-CSSN-TRAN-ID                  PIC X(4)  VALUE 'CSSN'.
       77  C-CESL-TRAN-ID                  PIC X(4)  VALUE 'CESL'.
       77  C-CSSF-TRAN-ID                  PIC X(4)  VALUE 'CSSF'.
      ...
      * EIBRESP2 values
       ...
       77  C-PASSWORD-FORMAT-INVALID       PIC S9(4) COMP VALUE 4.
       77  C-INVALID-USERID                PIC S9(4) COMP VALUE 8.
       77  C-TERMID-ALREADY-SIGNED-ON      PIC S9(4) COMP VALUE 9.
       77  C-INVALID-REQUEST               PIC S9(4) COMP VALUE 13.
      ...
      * made new eibresp2 codes
       ...
       77  C-OPERATOR-ACCOUNT-EXPIRED      PIC S9(4) COMP VALUE 512.
      ...
      *************************
      * pass-phrase variables *
      *************************
       01 LENS.
         05 PHR-LEN         PIC 9(9) COMP VALUE 100.
         05 NEW-PHR-LEN     PIC 9(9) COMP VALUE 100.
         05 CH-CTR     PIC S9(4) COMP VALUE 100.
       01 CESL-WRK.
          02 PASS-PHR.
             05 PASSPHR1    PIC X(50).
             05 PASSPHR2    PIC X(50).
      *************************
          02 PWCHR1       REDEFINES PASS-PHR
                          PIC X(1)
                          OCCURS 100 TIMES.       
      *************************
          02 NEW-PASS-PHR.
             05 PASSPHR3    PIC X(50).
             05 PASSPHR4    PIC X(50).
      *************************
          02 PWCHR2       REDEFINES NEW-PASS-PHR
                          PIC X(1)
                          OCCURS 100 TIMES.       
      *************************
          02 NEW-PASS-PHR2.
             05 PASSPHR5    PIC X(50).
             05 PASSPHR6    PIC X(50).
      ...
      ******************
       ...
       01  KIX0458E.
           ...
           05  USER-ID         PIC X(14)  VALUE SPACES.
       ...
       01  KIX0464E.
           05  FILLER          PIC X(9)  VALUE 'KIX0464E '.
           05  APPL-ID         PIC X(8)  VALUE SPACES.
           05  FILLER          PIC X(30)
                  VALUE ' New password format err for: '.
           05  USER-ID         PIC X(14)  VALUE SPACES.
       ...
       01  KIX0542E.
           05  FILLER          PIC X(9)  VALUE 'KIX0542E '.
           05  APPL-ID         PIC X(4)  VALUE SPACES.
           05  FILLER          PIC X(26)
                  VALUE 'Account expired for User: '.
           05  USER-ID         PIC X(14)  VALUE SPACES.
           05  FILLER          PIC X(25)
                  VALUE ' - consult Security Admin'.
       ...
       01  KIX1752E.
           05  FILLER          PIC X(9)  VALUE 'KIX1752E '.
           05  APPL-ID         PIC X(8)  VALUE SPACES.
           05  FILLER          PIC X(10) VALUE ' Terminal '.
           05  TERM-ID         PIC X(4)  VALUE SPACES.
           05  FILLER          PIC X(43)
               VALUE ' No External Security Manager CESL disabled'.
      ...
      ********************************************* 
       GET-DATE-TIME SECTION.
           ...
           END-IF.
           IF EIBTRNID = C-CESL-TRAN-ID
              MOVE KIX-DATE TO ESLDATEO
              MOVE KIX-TIME TO ESLTIMEO
           END-IF.
           IF EIBTRNID = C-CSSN-TRAN-ID
       ...
       PROCESS-ERROR-CODE SECTION.
           ...
           EVALUATE EIBRESP2
               ...
               WHEN C-PASSWORD-EXPIRED
                       ...
                       MOVE USERIDX TO USER-ID OF KIX0496E
                       MOVE KIX0496E TO MSG

               WHEN C-OPERATOR-ACCOUNT-EXPIRED
                       MOVE COMM-APPL-ID TO APPL-ID OF KIX0542E
                       MOVE USERIDX TO USER-ID OF KIX0542E
                       MOVE KIX0542E TO MSG

               WHEN C-PASSWORD-SUSPENDED
                       MOVE COMM-APPL-ID TO APPL-ID OF KIX0498E
                       MOVE USERIDX TO USER-ID OF KIX0498E
                       MOVE KIX0498E TO MSG

               WHEN C-PASSWORD-FORMAT-INVALID
                       MOVE COMM-APPL-ID TO APPL-ID OF KIX0464E
                       MOVE USERIDX TO USER-ID OF KIX0464E
                       MOVE KIX0464E TO MSG

               WHEN C-NEW-PASSWORD-NOT-VERIFIED
                       MOVE COMM-APPL-ID TO APPL-ID OF KIX0461E
                       MOVE USERIDX TO USER-ID OF KIX0461E
               ...
               WHEN C-INVALID-REQUEST
                       MOVE COMM-APPL-ID TO APPL-ID OF KIX1752E
                       MOVE EIBTRMID TO TERM-ID OF KIX1752E
                       MOVE KIX1752E TO MSG
      ...
      *************************************************************
           EXEC CICS HANDLE ABEND LABEL(ABEND-ROUTINE) END-EXEC.
           MOVE ESERIDI TO USERIDX.
           IF NEW-PASS-PHR NOT = SPACES
              EXEC CICS SIGNON
                        USERID(ESERIDI)
                        PHRASE(PASS-PHR)
                        PHRASELEN(PHR-LEN)
                        NEWPHRASE(NEW-PASS-PHR)
                        NEWPHRASELEN(NEW-PHR-LEN)
              END-EXEC
           ELSE
              EXEC CICS SIGNON
                        USERID(ESERIDI)
      ...
      ************************************************************
           ...
           IF EIBTRNID = C-CESL-TRAN-ID
              EXEC CICS
                   SEND MAP (C-CESL-MAP-NAME)
                        MAPSET(C-MAP-SET-NAME)
                        FROM(CESLMAPO)
                        LENGTH(LENGTH OF CESLMAPO)
                        ERASE
                        FREEKB
              END-EXEC
           END-IF.
      ...
      * must be tranid 'cssn'
           ...
           IF EIBTRNID = C-CESL-TRAN-ID
              EXEC CICS
                   RECEIVE MAP (C-CESL-MAP-NAME)
                           MAPSET(C-MAP-SET-NAME)
                           INTO(CESLMAPI)
                           ASIS
              END-EXEC
              MOVE PASSPH1I TO PASSPHR1
              MOVE PASSPH2I TO PASSPHR2
              MOVE PASSPH3I TO PASSPHR3
              MOVE PASSPH4I TO PASSPHR4
              MOVE PASSPH5I TO PASSPHR5
              MOVE PASSPH6I TO PASSPHR6
              PERFORM CALC-LEN
           END-IF.
      ...
      ********************************************************** 
      *  Determine precise phrase length
      ********************************************************** 
       CALC-LEN SECTION.
           MOVE 100 TO CH-CTR.
       LOOP.
           IF PWCHR1(CH-CTR) = ' '
              SUBTRACT 1 FROM CH-CTR
              GO TO LOOP
           ELSE
           MOVE CH-CTR TO PHR-LEN
           END-IF.
      ...
      ***************************************************************
           ...
           END-IF.
           IF EIBTRNID = C-CESL-TRAN-ID
                   GO TO SIGN-ON-USER-CESL-EXIT
           END-IF.
           IF EIBTRNID = C-CSSN-TRAN-ID
                   GO TO SIGN-ON-USER-CSSN-EXIT
           END-IF.
           IF EIBTRNID = C-CSSF-TRAN-ID
