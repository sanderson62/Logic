...
/* $Date:   16 Apr 2013 15:31:58  $ */
/* $Modtime:   16 Apr 2013 15:31:54  $ */

#ifndef lint
#ifdef __STDC__
const
#endif
static char sccsid[] = "@(#) $Workfile:   kxusrexit.c  $ $Revision:   1.12  $";
#endif

/*
 * $Log:   /home/dd134127/unikix/TPE12.0/unikixsrc/user/trans/PVCS/kxusrexit.c_v  $
 *
 *    Rev 1.12   16 Apr 2013 15:31:58   dd134127
 * Merge the following revisions:
 * TPE12.0 - 1.11
 * 7002141 - 1.9.2.0
 * 7002454 - 1.10.1.0
 *
 *    Rev 1.11   18 Jan 2013 08:56:46   unikix
 * Dell rebranding
 *
 *    Rev 1.10.1.0   01 Feb 2013 11:16:30   dd134127
 * B7002454
 * kixinstall fails with MF COBOL and DB2
 *
 *    Rev 1.10   30 Oct 2012 13:03:08   mm136118
 * B7002292
 * Fix MF COBOL/ORACLE database build problems
 *
 *    Rev 1.9.2.0   27 Feb 2013 09:45:42   dd134127
 * D7002141
 * Remove unsupported third-party products from TPE
 *
 *    Rev 1.9.1.0   11 Jan 2013 15:19:10   jk136119
 * B7002507 - If db connection not successful, abend batch program.
 * .
 *
...
#if defined(VCOBOL) || defined(KX_SERVER_EXPRESS)
#define USE_KXCOBLOAD
#endif

#ifndef WIN_NATIVE		/* suppress MF Server Express specific functionality; unsupported on native Windows */
#include <sys/wait.h>
#endif
#include "cics_eib.h"
/*----------------------------------------------------------------------*/
...
extern void kxSet_EZH_Tranid();
extern char kxGetEZHkey();
...
#define E_1007          1007     /*  Start Txn - reconnect */
...
/************************************************************************/
/*  Define user functions prototypes ORACLE, DBTWO, ESQL,               */
/*  MSSQL, and MQSERIES                                                 */
...
/* Case # 2388 */
#define MAX_AM ORACLE_AM + MSSQL_AM + DBTWO_AM + ESQL_AM + MQSERIES_AM
...
/************************************************************************/
/*  Define the occurrences of access method user exits and initialize   */
/*  the prototype ORACLE, DBTWO, etc. function addresses.               */
...
static int rcode;
static int errind;
int kxusrexit_batch_mode = 0; /* will be set to E_PRINTF by runner.c */
/* Case 2388 */
#if defined ORACLE || defined MSSQL \
    || defined DBTWO || defined ESQL  || defined MQSERIES
...
/* vCOBOL, and may be the case for other things in the future.              */
#ifdef USE_KXCOBLOAD
...
/************************************************************************/
/*                                                                      */
/*   The following code is specific to the support of vCOBOL ESQL       */
/*   database access.                                                   */
/*   The routines called by the functions below can be found in the     */
/*   KXESQL.cbl source file which is located in the $UNIKIX/src         */
/*   directory.                                                         */
/*                                                                      */
/************************************************************************/
...
{
   return kxcobload("KXESQL", KXDEBUG_NONE, 0, NULL, 0, LOGERR, NULL, 0);
}
...
db2_null_func()
{
   sqlgmf(); /* need reference to sqlgmf for linking with MF COBOL */
}
#endif

#if defined VCOBOL && defined RS6000
db2_null_func2()
{
   sqlgstrt(); /* need a reference into libdb2.a */
}
#endif
...
int kxuser_allocate()
{
   ...
   if (kxdbcheck(MAX_AM + 1, 0)) {
      kxerror(E_964, kxusrexit_batch_mode + E_PRINT, "kxuser_allocate");
      return(KXFAILURE);
   }
   for (i = 0; i < MAX_AM; i++) {
      /* ------------------ ---------------------------------
      ...
      if ( g_rdbms_bypass == 0) {
         if ((lrdbms_bypass = getenv("KIXRDBMS_BYPASS")) != NULL) {
            kxerror(E_1097, kxusrexit_batch_mode + E_PRINT, "kxuser_allocate");
            g_rdbms_bypass = 1;
         } else {
            g_rdbms_bypass = 0;
         }
      ...
      if ((kxexits[i].user_start_txn != NULL) &&
         ...
         do {
            if (g_rdbms_connected[i] == 0) { /* no connection */
               ...
               errind = kxusr_chkerr(rcode, E_1007);
               xexit_state = START;
               if (rcode != KXSUCCESS) {
                  g_rdbms_connected[i] = 0; /* connection failed */
               } else {
...
#endif
{
   ...
   if (prcode == KXCOBOLABORT) { /* Fatal error */
      kxerror1(E_1099, kxusrexit_batch_mode + E_PRINT, (char *)CurFunc, rcode);
      lerrind = KXABORT;
   } else if (prcode == KXSUCCESS) { /* successful rollback */
#ifdef TRACEMSG
      kxerrorstr(pmsgnbr, kxusrexit_batch_mode + E_PRINT, (char *)CurFunc, kxexits[i].user_rdbms);
#else
      if ((pmsgnbr == E_1001) || (pmsgnbr == E_1002) || (pmsgnbr == E_1007)) {
         kxerrorstr(pmsgnbr, kxusrexit_batch_mode + E_PRINT, (char *)CurFunc, kxexits[i].user_rdbms);
      }
#endif /* TRACEMSG */
      lerrind = KXSUCCESS;
...
#ifndef WIN_NATIVE		/* suppress unsupported MF Server Express function on native Windows */
/********************************************************************/
...
#endif
         {
            kxerror2(E_5104, kxusrexit_batch_mode + E_PRINT, "debug", mypid, errno);
         } else {
            if(WIFEXITED(pidstatus)) {
               childstatus = WEXITSTATUS(pidstatus);
               if(childstatus != 0) {
                  kxerror2(E_5105, kxusrexit_batch_mode + E_PRINT, "debug", mypid, childstatus);
               }
            } else {
               sigstatus = WTERMSIG(pidstatus);
               kxerror2(E_5106, kxusrexit_batch_mode + E_PRINT, "debug", mypid, sigstatus);
            }
         }
      } else if(pchild == 0) { /* child */
         execl(db, "kxchg_dbg", (char *)0);
         kxerror2str(E_5107, kxusrexit_batch_mode + E_PRINT, "debug", db, mypid, errno);
         exit(-1);
      } else {              /* fork failed */
         kxerror2(E_5108, kxusrexit_batch_mode + E_PRINT, "debug", mypid, errno);
      }
...
void kxzxcatt(char pAid)
{
static char lEZHtxn[4];
static int EZH_checked = 0;
static char lEZHkey = 0;
char *lenv;

    if (EZH_checked == 0) {
        if ( (lenv = getenv("EZHHELPTXN")) == NULL) {
	    memcpy(lEZHtxn, "    ", 4);
	    lEZHkey = 0;
	} else {
	    memcpy(lEZHtxn, lenv, 4);
		if ( (lenv = getenv("EZHKEY")) == NULL) {
		lEZHkey = 0;
	    } else {
		lEZHkey = kxGetEZHkey(lenv);
	    }
	}
	EZH_checked = 1;
    }
