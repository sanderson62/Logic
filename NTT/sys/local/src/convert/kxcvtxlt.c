...
extern void kxDBCStranslate(unsigned char *buf, int len, char conversion);
   ...
   */
   ...
   if(!strncmp(petype, "52", 2)) {
      ...
      if(direction == OUTBOUND) {
         ...
         kxDBCStranslate((unsigned char *)pdata, length, 'E');

      } else {

         kxDBCStranslate((unsigned char *)pdata, length, 'A');
