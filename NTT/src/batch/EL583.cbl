...
111315* 111315  IR2015111100001  PEMA  remove post of interface records
...
061814******************************************************************
061814
111315*    if (dte-client = 'DCC')
111315*       and (parm-company-cd = 'DCC' OR 'CSI' OR 'CCC')
111315*       perform 5000-build-sql-table
111315*                                thru 5000-exit
111315*    end-if
...
061814         move dc-greg-date-a-edit     to tbl-date
      ...
      ********* COMPANY CODE OF HEX '04' = CID
           ...
           END-IF
111709
011116     IF PARM-COMPANY-CD = 'DCC' or 'VPP'
011116        IF CQ-CARRIER = '1' OR '2' or '9'
111709          CONTINUE
111709        ELSE
...
112216        IF CQ-CARRIER = '3' or '4' or '5' OR '6' or '7'
111709          CONTINUE
111709     ELSE
111709          GO TO 2000-EXIT
111709        END-IF
...
061617     MOVE CQ-COMPANY-CD          TO AT-COMPANY-CD
061617     MOVE CQ-CARRIER             TO AT-CARRIER
061617     MOVE CQ-CLAIM-NO            TO AT-CLAIM-NO
061617     MOVE CQ-CERT-NO             TO AT-CERT-NO
061617     MOVE CQ-PMT-TRLR-SEQUENCE   TO AT-SEQUENCE-NO
061617     READ ELTRLR
061617     if eltrlr-file-status = '00'
061617        if at-ach-payment = 'Y'
061617           move 'ACHPMNT'        TO WS-D1-CHECK-NUMBER
061617        END-IF
061617     END-IF
...
061814
      ...
      *                                to p-sql-server
           ...
           move 'FN_GLDailyBalancing'  to p-sql-database
...
061814     EXEC SQL
NTTDel*        truncate table DAILYGLINT
NTTIns         EXECUTE IMMEDIATE 'truncate table DAILYGLINT'
061814     END-EXEC
061814     if sqlcode not = 0
...
011116        when parm-company-cd = 'DCC' or 'VPP'
