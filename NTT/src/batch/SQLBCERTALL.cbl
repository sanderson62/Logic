       ...
       PROGRAM-ID.    SQLBCERTALL.
       AUTHOR.        Cowtown.
       DATE-COMPILED.

      *REMARKS. This program builds the DCC_CERTALL table
      * every month.
...
081017* 081017  IR2017072400001  PEMA  Correct clp ref calc
...
070722     05  ext-cancel-reason       pic x.               
       ...
       PROCEDURE DIVISION.
           ...
           display ' Begin Program SQLBCERTALL '
...
070722     MOVE CR-CANCEL-REASON       TO EXT-CANCEL-REASON
           ...
           IF ERMAIL-FILE-STATUS = '00'
              ...
              move ma-state            to ext-ins-state
...
081017        move zeros               to ws-acct-fee     
081017                                    ws-cso-admin-fee
081017                                    ws-ga-fee       
              perform varying a1 from +1 by +1 until a1 > +10
                 evaluate true
                    when cr-agt-type (a1) = 'C' or 'D'
      ...
      *    display ' details ' cr-carrier ' ' cr-state ' ' cr-account
      *    ' ' cr-dt ' ' cr-cert-no ' ' cr-ah-clp ' ' cr-ah-rfnd-clp
      *    ' ' ws-acct-fee ' ' ws-cso-admin-fee ' ' ws-ga-fee ' '
      *    cr-ahrfnd ' ' ws-clp-prem
...
NTTIns*          USING    :pass
      *     END-EXEC
       ...
       1020-truncate-table.
           ...
           EXEC SQL
NTTDel*        truncate table DCC_CERTALL
NTTIns         EXECUTE IMMEDIATE 'truncate table DCC_CERTALL'
           END-EXEC
      ...
      *          AH_ORIG_CLP        decimal(7, 2) NULL,
      *          AH_ORIG_RFND_CLP   decimal(7, 2) NULL,
                 LF_CLP             decimal(7, 2) NULL,
                 LF_RFND_CLP        decimal(7, 2) NULL
      *          LF_ORIG_CLP        decimal(7, 2) NULL,
      *          LF_ORIG_RFND_CLP   decimal(7, 2) NULL
                 CONSTRAINT PK_DCC_CERTALL PRIMARY KEY CLUSTERED
                   (MOE_DATE, CARRIER, GROUPING, CLP_STATE, STATE,
...
070722           CANCEL_REASON)
      ...
      *          :EXT-MOE-DATE         ,
                 ...
                 :EXT-LF-REFUND-a      ,
                 :EXT-LF-EXP-DT :nu-lf-exp-dt       ,
                 :EXT-LF-CAN-DT :nu-lf-can-dt        ,
                 :EXT-LF-CAN-EXT-DT :nu-lf-can-ext-dt ,
                 :EXT-LF-DTH-DT :nu-lf-dth-dt ,
                 :EXT-LF-DTH-EXT-DT :nu-lf-dth-ext-dt,
                 :EXT-AH-BEN           ,
                 :EXT-AH-CUR-STATUS    ,
                 :EXT-AH-TERM          ,
                 :EXT-AH-AMT-a         ,
                 ...
                 :EXT-AH-CRIT-PER      ,
                 :EXT-AH-REFUND-a      ,
                 :EXT-AH-EXP-DT :nu-ah-exp-dt,
                 :EXT-AH-CAN-DT :nu-ah-can-dt,
                 :EXT-AH-CAN-EXT-DT :nu-ah-can-ext-dt,
                 :EXT-AH-DIS-DT :nu-ah-dis-dt,
                 :EXT-AGT1             ,
                 :EXT-TYP1             ,
                 :EXT-LF-COM1-a        ,
                 :EXT-AH-COM1-a        ,
                 ...
                 :EXT-ADDL-FEES-A      ,
      *          :EXT-ACT-AH-CLP-A     ,
      *          :EXT-ACT-AH-RFND-CLP-A,
                 :EXT-LF-CLP-A         ,
070722           :EXT-LF-RFND-CLP-A    ,
      *          :EXT-ACT-LF-CLP-A     ,
070722*          :EXT-ACT-LF-RFND-CLP-A,
070722           :EXT-CANCEL-REASON)
       ...
       1045-check-dates.
           ...
           if ext-lf-exp-dt = spaces
              move -1                  to nu-lf-exp-dt
           else
              move +0                  to nu-lf-exp-dt
           end-if
           if ext-lf-can-dt = spaces
              move -1                  to nu-lf-can-dt
           else
              move +0                  to nu-lf-can-dt
           end-if
           if ext-lf-can-ext-dt = spaces
              move -1                  to nu-lf-can-ext-dt
           else
              move +0                  to nu-lf-can-ext-dt
           end-if
           if ext-lf-dth-dt = spaces
              move -1                  to nu-lf-dth-dt
           else
              move +0                  to nu-lf-dth-dt
           end-if
           if ext-lf-dth-ext-dt = spaces
              move -1                  to nu-lf-dth-ext-dt
           else
              move +0                  to nu-lf-dth-ext-dt
           end-if
           if ext-ah-exp-dt = spaces
              move -1                  to nu-ah-exp-dt
           else
              move +0                  to nu-ah-exp-dt
           end-if
           if ext-ah-can-dt = spaces
              move -1                  to nu-ah-can-dt
           else
              move +0                  to nu-ah-can-dt
           end-if
           if ext-ah-can-ext-dt = spaces
              move -1                  to nu-ah-can-ext-dt
           else
              move +0                  to nu-ah-can-ext-dt
           end-if
           if ext-ah-dis-dt = spaces
              move -1                  to nu-ah-dis-dt
           else
              move +0                  to nu-ah-dis-dt
           end-if
